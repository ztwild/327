...
uint32_t io_teleport_pc(dungeon *d)
{
  ...
  mvprintw(0, 0, "Choose a location.  'g' to teleport to; 'r' for random.");
...
void look_at_monster(dungeon *d){
  ...
  do{
    ...
    case '7': case 'y': case KEY_HOME:
      if(cur[dim_x] > 1) cur[dim_x]--;
      break;
    case '3': case 'n': case KEY_NPAGE:
    case '6': case 'l': case KEY_RIGHT:
    case '9': case 'u': case KEY_PPAGE:
      if(cur[dim_x] < DUNGEON_X - 1) cur[dim_x]++;
      break;
    case 'g':
      c = charpair(cur);
      if (c && c != d->PC &&
